...
final class MockNewsFailureService: NewsServiceProtocol {
    ...
    func getNews(completion: @escaping (Result<[News]>) -> Void) {
        completion(.failure(.networkError(internal: MockError())))
    }
}

private class MockError: Swift.Error {
